...
def sync_stream(sf, catalog_entry, state, state_msg_threshold):
    ...
    with metrics.record_counter(stream) as counter:
        try:
            ...
            # Removed raising exception. Instead logging it to handle this inside of aws step functions
            # raise Exception("Error syncing {}: {} Response: {}".format(stream, ex, ex.response.text))
            LOGGER.error("Error syncing {}: {} Response: {}".format(stream, ex, ex.response.text))
        except Exception as ex:
            # Removed raising exception. Instead logging it to handle this inside of aws step functions
            #raise Exception("Error syncing {}: {}".format(stream, ex)) from ex
            LOGGER.error("Error syncing {}: {}".format(stream, ex))
